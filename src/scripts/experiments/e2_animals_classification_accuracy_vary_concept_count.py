...
PRETRAINED_MODEL_SAVE_PATH = os.getenv("MODEL_FOLDER") + f"/eeg_img_classification_training_e2_{MODEL}.pt"
PRETRAINED_LOSS_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/losses/eeg_img_classification_pretraining_e2_{MODEL}_losses.npy"
FINETUNING_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "training" / "eeg_img_classification_finetuning.yaml"
FINETUNED_MODEL_SAVE_PATH = os.getenv("MODEL_FOLDER") + f"/eeg_img_classification_training_e2_{MODEL}.pt"
FINETUNED_LOSS_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/losses/eeg_img_classification_finetuning_e2_{MODEL}_losses.npy"
CLASSIFICATION_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "inference" / "eeg_classification.yaml"
RESULT_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/experiment_results/eeg_classification_e2_{MODEL}_results_v1.json"
...
def run_training(train_dataset, limit_to_idxs=None, run_pretraining=True):
...
def evaluate(test_dataset, limit_to_idxs: [list, None]=None):
...
if __name__ == "__main__":
    ...
    for model in ["Benchmark", "LSTM", "EEGNETV4", "EEGChannelNet",   "EEGChannelNetGraphAttention"]:
        MODEL = model
        ...
        for subject in subjects:
            PRETRAINING_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "training" / "eeg_img_classification_pretraining.yaml"
            PRETRAINED_MODEL_SAVE_PATH = os.getenv("MODEL_FOLDER") + f"/eeg_img_classification_training_e2_no_finetuning_s{subject}_{MODEL}.pt"
            PRETRAINED_LOSS_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/losses/eeg_img_classification_pretraining_e2_no_finetuning_s{subject}_{MODEL}_losses.npy"
            FINETUNING_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "training" / "eeg_img_classification_finetuning.yaml"
            FINETUNED_MODEL_SAVE_PATH = os.getenv("MODEL_FOLDER") + f"/eeg_img_classification_training_e2_no_finetuning_s{subject}_{MODEL}.pt"
            FINETUNED_LOSS_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/losses/eeg_img_classification_finetuning_e2_no_finetuning_s{subject}_{MODEL}_losses.npy"
            CLASSIFICATION_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "inference" / "eeg_classification.yaml"
            RESULT_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/experiment_results/finetuning_s{subject}/eeg_classification_e2_no_finetuning_s{subject}_{MODEL}_results_v1.json"
            ...
            possible_concepts = list(range(0, 200))
            ...
            training_cases = [
                ...
                (100, 1),
                (200, 1),
                (len(possible_concepts), 1),
            ]
            results = []

            run_pretraining = True
            for i, (concept_count, repetitions) in enumerate(training_cases):
                concept_count_results = []
                for j in range(repetitions):
                    print("Training case ", i + 1, " repetition ", j + 1)
                    concepts_sampled = random.sample(possible_concepts, concept_count)
                    train_dataset = prepare_training_dataset(None)
                    test_dataset = prepare_test_dataset(concepts_sampled)
                    #train_idxs, test_idxs =  train_test_split(range(len(dataset)), train_size=TRAIN_SIZE)
                    run_training(train_dataset, None, run_pretraining)
                    run_pretraining = False
                    concept_count_results.append(evaluate(test_dataset, None))
                result = {
                    "concept_count": concept_count,
                    "repetitions": repetitions,
                    "results": concept_count_results
                }
