...
MODEL = "EEGChannelNetSelfAttention"
LATENT = "linguistic"
FINETUNING = True
...
PRETRAINED_MODEL_SAVE_PATH = os.getenv("MODEL_FOLDER") + f"/eeg_img_classification_training_e1_{MODEL}.pt"
PRETRAINED_LOSS_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/losses/eeg_img_classification_pretraining_e1_{MODEL}_losses.npy"
FINETUNING_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "training" / "eeg_img_classification_finetuning.yaml"
FINETUNED_MODEL_SAVE_PATH = os.getenv("MODEL_FOLDER") + f"/eeg_img_classification_training_e1_{MODEL}.pt"
FINETUNED_LOSS_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/losses/eeg_img_classification_finetuning_e1_{MODEL}_losses.npy"
CLASSIFICATION_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "inference" / "eeg_classification.yaml"
RESULT_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/experiment_results/eeg_classification_e1_{MODEL}_results_v1.json"
LIMIT_TO_CLASSES = [
            'toy',
            'unknown',
            'food',
...
#sample_count = [520, 10759, 3400, 760, 920, 880, 640, 920, 1880, 1160, 1440, 880, 960, 280, 920, 320, 800, 1600, 480, 40, 560, 600, 600, 400, 320, 320, 720]
img_classes = [
            'toy',
            'unknown',
            'food',
...
def prepare_training_dataset():
    ...
    classification_dataset = ImageClassEEGEncoderTrainingDataset(
...
def evaluate(test_dataset, limit_to_idxs=None):
    ...
    for i in range(length):
        ...
        if true_class == output_class:
            class_statistics[LIMIT_TO_CLASSES[true_class]]["true_positives"] += 1
            for c in LIMIT_TO_CLASSES:
                class_statistics[c]["true_negatives"] += 1
        else:
            class_statistics[LIMIT_TO_CLASSES[true_class]]["false_negatives"] += 1
            class_statistics[LIMIT_TO_CLASSES[output_class]]["false_positives"] += 1
            for c in LIMIT_TO_CLASSES:
                if c != LIMIT_TO_CLASSES[true_class] and c != LIMIT_TO_CLASSES[output_class]:
                    class_statistics[c]["true_negatives"] += 1
    
    for c in LIMIT_TO_CLASSES:
        true_positives = class_statistics[c]["true_positives"]
        false_positives = class_statistics[c]["false_positives"]
        true_negatives = class_statistics[c]["true_negatives"]
        false_negatives = class_statistics[c]["false_negatives"]
        class_statistics[c]["accuracy"] = (true_positives + true_negatives) / (true_positives + true_negatives + false_positives + false_negatives) if (true_positives + true_negatives + false_positives + false_negatives) > 0 else 0
        class_statistics[c]["precision"] = true_positives / (true_positives + false_positives) if (true_positives + false_positives) > 0 else 0
        class_statistics[c]["recall"] = true_positives / (true_positives + false_negatives) if (true_positives + false_negatives) > 0 else 0
        class_statistics[c]["f1"] = 2 * (class_statistics[c]["precision"] * class_statistics[c]["recall"]) / (class_statistics[c]["precision"] + class_statistics[c]["recall"]) if (class_statistics[c]["precision"] + class_statistics[c]["recall"]) > 0 else 0

    mean_accuracy = sum([c["accuracy"] for c in class_statistics.values()]) / len(class_statistics)
    mean_precision = sum([c["precision"] for c in class_statistics.values()]) / len(class_statistics)
    mean_recall = sum([c["recall"] for c in class_statistics.values()]) / len(class_statistics)
    mean_f1 = sum([c["f1"] for c in class_statistics.values()]) / len(class_statistics)
...
if __name__ == "__main__":
    ...
    for model in ["Benchmark", "EEGChannelNetSelfAttention", "EEGChannelNet", "EEGNETV4", "EEGChannelNetGraphAttention", "LSTM"]:
        MODEL = model
        ...
        for subject in subjects:
            PRETRAINING_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "training" / "eeg_img_classification_pretraining.yaml"
            PRETRAINED_MODEL_SAVE_PATH = os.getenv("MODEL_FOLDER") + f"/eeg_img_classification_training_e1_no_finetuning_s{subject}_{MODEL}.pt"
            PRETRAINED_LOSS_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/losses/eeg_img_classification_pretraining_e1_no_finetuning_s{subject}_{MODEL}_losses.npy"
            FINETUNING_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "training" / "eeg_img_classification_finetuning.yaml"
            FINETUNED_MODEL_SAVE_PATH = os.getenv("MODEL_FOLDER") + f"/eeg_img_classification_training_e1_no_finetuning_s{subject}_{MODEL}.pt"
            FINETUNED_LOSS_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/losses/eeg_img_classification_finetuning_e1_no_finetuning_s{subject}_{MODEL}_losses.npy"
            CLASSIFICATION_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "inference" / "eeg_classification.yaml"
            RESULT_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/experiment_results/finetuning_s{subject}/eeg_classification_e1_no_finetuning_s{subject}_{MODEL}_results_v1.json"
