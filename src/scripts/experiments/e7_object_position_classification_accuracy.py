...
PRETRAINED_MODEL_SAVE_PATH = os.getenv("MODEL_FOLDER") + f"/eeg_img_classification_pretraining_e7_{MODEL}.pt"
PRETRAINED_LOSS_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/losses/eeg_img_classification_pretraining_e7_{MODEL}_losses.npy"
FINETUNING_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "training" / "eeg_img_classification_finetuning.yaml"
FINETUNED_MODEL_SAVE_PATH = os.getenv("MODEL_FOLDER") + f"/eeg_img_classification_finetuning_e7_{MODEL}.pt"
FINETUNED_LOSS_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/losses/eeg_img_classification_finetuning_e7_{MODEL}_losses.npy"
CLASSIFICATION_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "inference" / "eeg_classification.yaml"
RESULT_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/experiment_results/eeg_classification_e7_{MODEL}_results_v1.json"
SUBJECTS = [1]
RUN_TRAINING = True
EPOCHS = 100
TRAIN_SIZE = 0.8
...
def prepare_training_dataset():
    ...
    classification_dataset = ObjectPositionEEGEncoderTrainingDataset(
...
def evaluate(test_dataset, limit_to_idxs=None):
    ...
    for i in range(length):
        ...
        if true_class == output_class:
            position_statistics[true_class]["true_positives"] += 1
            for c in range(len(POSITIONS)):
                if c != true_class:
                    position_statistics[c]["true_negatives"] += 1
        else:
            position_statistics[true_class]["false_negatives"] += 1
            position_statistics[output_class]["false_positives"] += 1
            for c in range(len(POSITIONS)):
                if c != true_class and c != output_class:
                    position_statistics[c]["true_negatives"] += 1

    for c in range(len(POSITIONS)):
        true_positives = position_statistics[c]["true_positives"]
        false_positives = position_statistics[c]["false_positives"]
        true_negatives = position_statistics[c]["true_negatives"]
        false_negatives = position_statistics[c]["false_negatives"]
        position_statistics[c]["accuracy"] = (true_positives + true_negatives) / (true_positives + true_negatives + false_positives + false_negatives) if (true_positives + true_negatives + false_positives + false_negatives) > 0 else 0
        position_statistics[c]["precision"] = true_positives / (true_positives + false_positives) if (true_positives + false_positives) > 0 else 0
        position_statistics[c]["recall"] = true_positives / (true_positives + false_negatives) if (true_positives + false_negatives) > 0 else 0
        position_statistics[c]["f1"] = 2 * (position_statistics[c]["precision"] * position_statistics[c]["recall"]) / (position_statistics[c]["precision"] + position_statistics[c]["recall"]) if (position_statistics[c]["precision"] + position_statistics[c]["recall"]) > 0 else 0
    
    mean_accuracy = sum([c["accuracy"] for c in position_statistics]) / len(position_statistics)
    mean_precision = sum([c["precision"] for c in position_statistics]) / len(position_statistics)
    mean_recall = sum([c["recall"] for c in position_statistics]) / len(position_statistics)
    mean_f1 = sum([c["f1"] for c in position_statistics]) / len(position_statistics)
    ...
    return {
        ...
        "mean_f1": mean_f1,
        "color_statistics": position_statistics,
    }
...
if __name__ == "__main__":
    ...
    for model in ["Benchmark", "EEGNETV4", "EEGChannelNetSelfAttention", "EEGChannelNet", "EEGChannelNetGraphAttention", "LSTM"]:
        ...
        for subject in subjects:
            PRETRAINING_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "training" / "eeg_img_classification_pretraining.yaml"
            PRETRAINED_MODEL_SAVE_PATH = os.getenv("MODEL_FOLDER") + f"/eeg_img_classification_training_e7_no_finetuning_s{subject}_{MODEL}.pt"
            PRETRAINED_LOSS_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/losses/eeg_img_classification_pretraining_e7_no_finetuning_s{subject}_{MODEL}_losses.npy"
            FINETUNING_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "training" / "eeg_img_classification_finetuning.yaml"
            FINETUNED_MODEL_SAVE_PATH = os.getenv("MODEL_FOLDER") + f"/eeg_img_classification_training_e7_no_finetuning_s{subject}_{MODEL}.pt"
            FINETUNED_LOSS_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/losses/eeg_img_classification_finetuning_e7_no_finetuning_s{subject}_{MODEL}_losses.npy"
            CLASSIFICATION_CONF_PATH = Path(os.getenv("CONFIG_FOLDER")) / "inference" / "eeg_classification.yaml"
            RESULT_SAVE_PATH = os.getenv("OUTPUT_FOLDER") + f"/experiment_results/finetuning_s{subject}/eeg_classification_e7_no_finetuning_s{subject}_{MODEL}_results_v1.json"
