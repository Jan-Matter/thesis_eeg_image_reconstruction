...
class ObjectWidthEEGEncoderTrainingDataset(Dataset):
    ...
    def num_classes(self):
        return len(self.__width_to_class_map) - 1
    ...
    def __getitem__(self, idx):
        ...
        if cache_path.exists():
            width, width_class = self.__load_from_cache(cache_path)
        else:
            ...
            width = self.__extract_object_width(foreground_img)
            width_class = self.__width_to_class(width)
            self.__save_to_cache(cache_path, width, width_class)
        ...
        return {
            ...
            'class': self.__width_to_class_map[width_class],
            'value': width,
    ...
    def __width_to_class(self, height):
        if height < 20:
            return 'unknown'
        if height < 100:
            return 'small'
    ...
    def __load_from_cache(self, cache_path):
        ...
        with open(cache_path, 'r') as f:
            data = json.load(f)
        self.__in_memory_cache[cache_path] = data['width'], data['width_class']
        return data['width'], data['width_class']
    
    
    def __save_to_cache(self, cache_path, width, width_class):
...
if __name__ == '__main__':
    ...
    dataset = ObjectWidthEEGEncoderTrainingDataset(
