...
class LinguisticEEGEncoderTrainingDataset(Dataset):
    ...
    def __init__(self,
        ...
        self.__latent_text_dataset = latent_text_dataset
    ...
    def num_classes(self):
        return len(self.__valid_concepts)
    ...
    def __getitem__(self, idx):
        ...
        #img_caption = next(self.__queryier.run_query(f"""SELECT image.img_concept FROM image WHERE image.img_id = {img_id}"""))['img_concept']
        latent = self.__latent_text_dataset.get_latent_img(img_id)
        return {
    ...
    def __init_idx_eeg_map(self):
        ...
        for i in range(len(self.__eeg_dataset)):
            ...
            if self.__limit_to_classes is None or img_class in self.__limit_to_classes:
                ...
                if self.__limit_to_concepts is None or concept in self.__limit_to_concepts:
                    if concept not in self.__valid_concepts:
                        self.__valid_concepts.append(concept)
                    eeg_data['img_concept'] = concept
    ...
    def __equalize_classes(self):
        ...
        for i in range(len(self)):
            eeg_data = self[i]
            curr_class = self.__valid_concepts.index(eeg_data['img_concept'])
...
if __name__ == '__main__':
    ...
    for data in dataloader:
        ...
        print(data['latent_text'].shape)
        print(data['img_caption'])
